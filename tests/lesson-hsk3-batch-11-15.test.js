...
test('HSK3 lessons 11-15 content pack exists and each lesson follows the expected shape', async () => {
  const batch = require('../lesson-hsk3-batch-11-15.js');

  assert.equal(batch.level, 'HSK3');
  assert.equal(Array.isArray(batch.lessons), true);
  assert.equal(batch.lessons.length, 5);
  assert.deepEqual(batch.lessons.map((lesson) => lesson.id), ["hsk3-l11","hsk3-l12","hsk3-l13","hsk3-l14","hsk3-l15"]);
