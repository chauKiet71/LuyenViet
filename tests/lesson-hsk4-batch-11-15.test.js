...
test('HSK4 lessons 11-15 content pack exists and each lesson follows the expected shape', async () => {
  const batch = require('../lesson-hsk4-batch-11-15.js');

  assert.equal(batch.level, 'HSK4');
  assert.equal(Array.isArray(batch.lessons), true);
  assert.equal(batch.lessons.length, 5);
  assert.deepEqual(batch.lessons.map((lesson) => lesson.id), ["hsk4-l11","hsk4-l12","hsk4-l13","hsk4-l14","hsk4-l15"]);
