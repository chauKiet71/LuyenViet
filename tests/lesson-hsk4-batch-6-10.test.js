...
test('HSK4 lessons 6-10 content pack exists and each lesson follows the expected shape', async () => {
  const batch = require('../lesson-hsk4-batch-6-10.js');

  assert.equal(batch.level, 'HSK4');
  assert.equal(Array.isArray(batch.lessons), true);
  assert.equal(batch.lessons.length, 5);
  assert.deepEqual(batch.lessons.map((lesson) => lesson.id), ["hsk4-l6","hsk4-l7","hsk4-l8","hsk4-l9","hsk4-l10"]);
