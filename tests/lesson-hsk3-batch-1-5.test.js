...
test('HSK3 lessons 1-5 content pack exists and each lesson follows the expected shape', async () => {
  const batch = require('../lesson-hsk3-batch-1-5.js');

  assert.equal(batch.level, 'HSK3');
  assert.equal(Array.isArray(batch.lessons), true);
  assert.equal(batch.lessons.length, 5);
  assert.deepEqual(batch.lessons.map((lesson) => lesson.id), ["hsk3-l1","hsk3-l2","hsk3-l3","hsk3-l4","hsk3-l5"]);
